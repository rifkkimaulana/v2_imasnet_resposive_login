...
(function (c) {
  ...
  c.widget("ui.tabs", {
    ...
    _tabId: function (d) {
      return (
        (d.title &&
          d.title.replace(/\s/g, "_").replace(/[^A-Za-z0-9\-_:\.]/g, "")) ||
        this.options.idPrefix + ++b
      );
    },
    ...
    _cookie: function () {
      var d =
        this.cookie ||
        (this.cookie = this.options.cookie.name || "ui-tabs-" + ++a);
    ...
    _tabify: function (q) {
      ...
      if (q) {
        ...
        f.selected =
          (f.selected >= 0 && this.anchors[f.selected]) || f.selected < 0
            ? f.selected
            : 0;
        f.disabled = c
          .unique(
        ...
        if (f.selected >= 0 && this.anchors.length) {
          ...
          r.element.queue("tabs", function () {
            r._trigger(
              "show",
              null,
              r._ui(r.anchors[f.selected], r.panels[f.selected])
            );
          });
      ...
      this.element[f.collapsible ? "addClass" : "removeClass"](
        "ui-tabs-collapsible"
      );
      if (f.cookie) {
      ...
      for (var j = 0, p; (p = this.lis[j]); j++) {
        c(p)[
          c.inArray(j, f.disabled) != -1 && !c(p).hasClass("ui-tabs-selected")
            ? "addClass"
            : "removeClass"
        ]("ui-state-disabled");
      }
    ...
    load: function (g) {
      ...
      if (
        !f ||
        (this.element.queue("tabs").length !== 0 && c.data(d, "cache.tabs"))
      ) {
